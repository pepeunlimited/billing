...
func (iq *ItemQuery) QueryOrders() *OrdersQuery {
	query := &OrdersQuery{config: iq.config}
	step := sqlgraph.NewStep(
		sqlgraph.From(item.Table, item.FieldID, iq.sqlQuery()),
...
func (iq *ItemQuery) WithOrders(opts ...func(*OrdersQuery)) *ItemQuery {
	query := &OrdersQuery{config: iq.config}
...
func (iq *ItemQuery) sqlAll(ctx context.Context) ([]*Item, error) {
	...
	if query := iq.withOrders; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Item)
		for i := range nodes {
			if fk := nodes[i].orders_items; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "orders_items" returned %v`, n.ID)
			}
			for i := range nodes {
