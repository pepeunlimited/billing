...
func (tq *TxsQuery) QueryOrders() *OrdersQuery {
	query := &OrdersQuery{config: tq.config}
...
func (tq *TxsQuery) WithOrders(opts ...func(*OrdersQuery)) *TxsQuery {
	query := &OrdersQuery{config: tq.config}
...
func (tq *TxsQuery) sqlAll(ctx context.Context) ([]*Txs, error) {
	...
	if query := tq.withOrders; query != nil {
		...
		for i := range nodes {
			if fk := nodes[i].orders_txs; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "orders_txs" returned %v`, n.ID)
			}
			for i := range nodes {
